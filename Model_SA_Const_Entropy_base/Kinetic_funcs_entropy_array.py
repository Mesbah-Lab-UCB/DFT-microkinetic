...
def auto_parse_to_kin(id):
    ...
    for i in range(id*10,(id+1)*10):
        ...
        subprocess.run('cd ' + ARRAY_TASK_DIR + '/{}_datastore && gfortran -O3 -ffree-line-length-none -o entropyV_base_{}.exe {}/dvode_f90_m.f90 zdplaskin_m_DFT_in_entropy_varying_basis_{}.F90 Const_E_{}.F90 {}/bolsig_x86_64.so'.format(i,i,SLURM_SUBMIT_DIR,i,i,SLURM_SUBMIT_DIR), shell=True)
        try:
            subprocess.run('cd ' + ARRAY_TASK_DIR + '/{}_datastore && export LD_LIBRARY_PATH=.:$LD_LIBRARY_PATH && ./entropyV_base_{}.exe'.format(i,i), shell=True, timeout=60*60, stdout=open(os.devnull, "w"))
        except:
            pass
